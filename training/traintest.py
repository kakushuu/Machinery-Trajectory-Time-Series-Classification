...
def train(audio_model, train_loader, test_loader, args):
    ...
    print('总参数数量: {:.3f} 百万'.format(sum(p.numel() for p in audio_model.parameters()) / 1e6))
    print('可训练参数数量: {:.3f} 百万'.format(sum(p.numel() for p in trainables) / 1e6))
    ...
    print('MLP头部使用 {:d} 倍的学习率'.format(args.head_lr))
    ...
    print('MLP参数数量: {:.3f} 百万'.format(sum(p.numel() for p in mlp_params) / 1e6))
    print('基础参数数量: {:.3f} 百万'.format(sum(p.numel() for p in base_params) / 1e6))
    ...
    while epoch < args.n_epochs + 1:
        ...
        if best_epoch == epoch:
            ...
            # 复制当前epoch的预测结果为最佳预测结果
            predictions_file = exp_dir+'/predictions/predictions_' + str(epoch) + '.csv'
            best_predictions_file = exp_dir+'/predictions/best_predictions.csv'
            if os.path.exists(predictions_file):
                predictions = np.loadtxt(predictions_file, delimiter=',')
                np.savetxt(best_predictions_file, predictions, delimiter=',')
                print('已保存最佳模型(epoch {})的预测结果到 {}'.format(epoch, best_predictions_file))
